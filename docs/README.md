...
- [x] `<할인 후 예상 결제 금액>`: `135,754원`(할인 전 총 주문금액 - 할인금액의 합계, 1000원 단위 쉼표)
- [x] `<12월 이벤트 배지>`: `산타`(총혜택금에데 따라 부여/ 5천원(별), 1만원(트리), 2만원(산타), `없음`
...
- [x] 총혜택 금액 = 할인 금액 총 합계 계산 + 증정품의 가격 or 0원 계산
- [x] 할인 후 예상 결제 금액: 할인 전 총주문 금액 - 할인금액 계산
- [x] Amount Enum 추가 (금액 관리 용도)
  - [x] description: 할인 전 총주문 금액, 총혜택 금액, 할인 후 예상 결제 금 객체
  - [x] initialValue: 초기값
  - [x] 금액 증가 메서드
  - [x] 객체끼리의 덧셈, 뺄셈 메서드
...
- [x] `Map<String, Integer>` -> 문제에서 제시한 주문 형식으로 변형 메서드